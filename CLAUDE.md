...
- **Period-scoped (`[periodStart, periodEnd)` half-open range):** New Customers / Pts Issued, Approvals, Pending stat card, the daily points chart (one bar per day of the month), the status donut, Top Rewards (approvals), and Top Customers (**points earned in the month**, aggregated from `earn` transactions — not the all-time `total_points` balance). **Every period-scoped query anchors on `created_at`** (not `resolved_at`), so all stats and charts in the period block stay mutually consistent for any selected month.
