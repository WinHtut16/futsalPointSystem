...
- **Booking logic:** `lib/booking.ts` — pure, timezone-safe (ISO `YYYY-MM-DD`) helpers: `priceForHour`/`tierForHour` (weekday AM 20k / PM 25k / weekend+holiday 30k), `isThingyan` (Apr 13–16, fallback for years not in holidays config), `isWeekendRate` (checks `isWeekend || isThingyan || isHoliday`), `dayHours` (16 slots 06:00–21:00), `depositFor` (flat 10,000 MMK per booking — always `DEPOSIT_PER_SLOT` regardless of slot count), `canCancel(bookingDate, hourStart)` (12-hour refund window — **Myanmar-timezone-aware**: subtracts `MYANMAR_OFFSET_MS` same as `isSlotBookable`; enforced server-side in `PATCH /api/bookings/[id]` for customer cancels; admin bypass skips this check), `isSlotBookable(dateISO, hourStart)` (returns false when slot start is < `BOOKING_LEAD_MINUTES=5` minutes away — accounts for Myanmar UTC+6:30 offset; enforced server-side in `POST /api/bookings` for non-override bookings; `BookingView` uses this to mark past/imminent available slots as `'closed'`), `MAX_SLOTS=2`. Unit-tested in `__tests__/booking-logic.test.ts`.
...
- **Admin manual booking:** `admin-booking-entry-migration.sql` makes `bookings.customer_id` nullable (guest bookings), adds `source TEXT CHECK ('online','phone','walk_in','other') DEFAULT 'online'`, `guest_name TEXT`, `guest_phone TEXT`, `internal_notes TEXT` columns, and creates `create_admin_booking_transaction(p_booking_date, p_slots, p_customer_id?, p_guest_name?, p_guest_phone?, p_deposit_total?, p_deposit_received?, p_source?, p_internal_notes?, p_is_override?)` RPC. **Required params come first** (`p_booking_date`, `p_slots`) — PostgreSQL requires all params after the first defaulted one to also have defaults. Admin-created bookings use `source ∈ {phone, walk_in, other}`; customer online bookings always use `source='online'`. Guest bookings require `guest_name` (non-empty); linked bookings require `customer_id`.
...
### Admin Installable PWA

Staff can install the admin panel as a home-screen/desktop app (Android, iOS, Windows, macOS) directly from the deployed URL — no store, no native wrapper. **Admin-only by design**: customers must never see an install prompt.

- **Manifest is a static file, not `app/manifest.ts`.** `public/pwa/manifest.webmanifest` is linked only via the `metadata.manifest` export in `app/(admin)/layout.tsx` — Next merges metadata top-down per route segment, so customer-facing route groups (`(site)`, `(customer)`, `(auth)`) never get the `<link rel="manifest">`, and `beforeinstallprompt` can never fire there. Do **not** move this to `app/manifest.ts`; that auto-injects the link site-wide and leaks the install prompt to customers. `scope: "/admin"` and `start_url: "/admin"` (the portal chooser — a billiards-only or game-only admin has no futsal grant) additionally confine the installed window itself.
- **This one manifest covers all three businesses.** The billiards and game admin panels are separate Next.js zones rewritten under this origin at `/admin/billiards` and `/admin/game` (see `next.config.js` rewrites); both link this same `/pwa/manifest.webmanifest` via their own `metadata.manifest` and register this same `/sw.js` at `{ scope: '/admin' }`, rather than shipping a manifest of their own. That is deliberate — a second in-scope manifest on the same origin is ambiguous — and is why `id` stays `/admin` regardless of which zone the install was triggered from: one icon, one installed app, for the whole admin suite.
- **Manifest/icons live under `/pwa/`, not `/admin/`.** `middleware.ts` treats any path starting with `/admin` as an admin route requiring a session; a manifest fetch carries no cookies, so serving it from `/admin/manifest.webmanifest` would 307 it to `/admin/login` and the browser would silently never see it. `config.matcher` explicitly excludes `sw.js` and `pwa/` from the middleware.
- **`public/sw.js` is a deliberate passthrough service worker** — registers (`install`/`activate` → `skipWaiting`/`clients.claim`) and has a no-op `fetch` listener (satisfies Chromium's installability check) but never calls `respondWith`, so it caches nothing. Every launch fetches fresh code from Vercel — **do not add caching here or auto-updates go stale.** It must live at the domain root (not `/pwa/`) because a worker can only claim a scope at or below its own script path; it's registered with `{ scope: '/admin' }`.
- **Install UI:** `components/admin/InstallAppCard.tsx`, mounted as a third section in `AdminProfileForm` (`/admin/profile`) — the only entry point, per client requirement (no banner, no sidebar item). Uses `hooks/usePwaInstall.ts` (`beforeinstallprompt` capture + iOS/macOS/standalone detection) to show either a native **Install App** button (Android/desktop Chromium) or a **How to Install** button opening a `Modal` with manual steps (iOS Safari: Share → Add to Home Screen; macOS Safari: File → Add to Dock; other browsers: use Chrome/Edge). Also performs the `navigator.serviceWorker.register('/sw.js', { scope: '/admin' })` call.
- **Icons:** `public/pwa/icon-{192,512}.png`, `icon-maskable-{192,512}.png`, `apple-touch-icon-180.png` — generated from `figures/logo_black.jpg` via `node scripts/generate-pwa-icons.mjs` (uses `sharp`, already a transitive Next.js dependency; no new package). Re-run this script if branding changes. `theme_color` in the manifest and `viewport.themeColor` in the admin layout must match `--color-primary` in `app/globals.css` (currently `#0b4327`).
- **Known limit:** iOS/iPadOS Safari lets a user "Add to Home Screen" on *any* page — Apple provides no way to opt a site out of this. Without an in-scope manifest, a customer doing this on a customer page gets a plain unbranded bookmark, not the admin app; that's the best achievable, not a bug.
...
**Site (booking) pages** (`app/(site)/`) — `SiteNavbar` is rendered **once** in `app/(site)/layout.tsx` (async server component — calls `getCurrentUser()`, passes `initialFirstName` to the navbar). Because layouts are persistent across client-side navigations, the navbar never re-mounts → zero auth flash. `loading.tsx` files in this route group **must NOT** include `<SiteNavbar>` — the layout instance stays visible during all loading states. `loading.tsx` files only render the content skeleton (plus `<BottomNav>` where the real page has one). Page content wrappers in `(site)` use the `.animate-page-in` CSS class (defined in `globals.css`) for a 150ms ease-out fade+slide entrance after loading resolves.
...
| `admin` | Add points, adjust points (correction/audit), manage customers (incl. delete), view all rewards, toggle rewards active/inactive, CMS (create/edit/delete posts + image uploads) |
...
| `lib/supabase/client.ts` | Browser Supabase client (for client components) — points at `browserSupabaseUrl()` (`/sb`, same-origin), not the direct Supabase URL |
| `lib/supabase/config.ts` | `SUPABASE_URL`/`SUPABASE_ANON_KEY`, `SUPABASE_PROXY_PATH='/sb'`, `browserSupabaseUrl()`, `SUPABASE_AUTH_COOKIE_NAME`. Some Myanmar mobile operators (confirmed: Ooredoo) reject direct browser connections to `*.supabase.co` outright in <300ms while server-to-server (Vercel→Supabase) is unaffected — see `app/net-check`. `next.config.js` rewrites `/sb/:path*` to the real Supabase origin so the browser never names `supabase.co`. **Cookie name is pinned identically on both `lib/supabase/client.ts` and `lib/supabase/server.ts`/`middleware.ts`** (`sb-<project-ref>-auth-token`, derived from the real `SUPABASE_URL` — matches supabase-js's own pre-existing default, so no session invalidation on deploy). If the browser and server ever derive different cookie names, every visitor is silently logged out — do not remove the explicit `cookieOptions: { name: SUPABASE_AUTH_COOKIE_NAME }` from any of the three call sites. **Trade-off:** Realtime WebSockets cannot follow `/sb` (Vercel does not upgrade protocol across a rewrite), so every Realtime consumer in this codebase (see Real-Time Architecture below) now runs on its polling fallback only — this is intentional, not a bug. |
| `lib/supabase/server.ts` | SSR Supabase client + `createServiceClient()` (raw `@supabase/supabase-js`, truly bypasses RLS) + `broadcastSlotChange(date)` — calls the Supabase Realtime REST broadcast API (no WebSocket) to notify `/book` clients after any booking or closure mutation; 2s timeout, errors swallowed |
...
**SQL prerequisites** — tracked in `realtime-publication-migration.sql` (idempotent, safe to re-run):
```sql
ALTER TABLE <table> REPLICA IDENTITY FULL;
ALTER PUBLICATION supabase_realtime ADD TABLE <table>;
```
Tables enabled: `bookings` (`booking-system-migration.sql`), `profiles` + `redemption_requests` (`realtime-publication-migration.sql`), `court_closures` (`realtime-closures-migration.sql`). REPLICA IDENTITY FULL set for all four.
...
| `useBookingSlotRefresh` (hook) | n/a — polling, not Realtime | 15s interval + `visibilitychange`/`focus` | Customer booking page — replaced the old `useBookingSlotRealtime` broadcast-channel hook (removed); polls only the currently-selected date via `refreshDate(dateISO)` (`/api/bookings/availability?date=…`, no loading state), patching `dayInfoState`/`calDataState` in `BookingView`. Realtime WebSockets cannot survive the `/sb` browser proxy (Vercel doesn't upgrade protocol across rewrites — see below), so this and every other Realtime consumer in this table now run on their polling fallback only; none open a working `postgres_changes`/broadcast connection from the browser in production. |
...
- `components/customer/account/` — unified account UI: **UnifiedAccount** (tab wrapper: **Upcoming · History · Points & Rewards** — tab keys: `'upcoming'` / `'history'` / `'points'`; accepts `initialTab?: 'upcoming' | 'history' | 'points'` prop (defaults `'upcoming'`) passed from the server page so `?tab=` deep-links land on the correct tab; History tab has per-filter cursor pagination state — `feeds`, `hasMore`, `cursors` (next cursor per filter) keyed by `all|bookings|points`; "Load more" button calls `GET /api/account/history?filter=...&before=CURSOR` and appends results; switching filter resets to server-provided initial data for that filter), **AccountHeader** (identity + flat live points strip — earn rate pill removed; logout button uses `border-red-200 text-red-500` — ghost/outline style, always visibly red; no tiers; `Settings` gear icon links to `/account/settings`), **AccountSettingsForm** (client — 3 sections: Profile info (`PATCH /api/profile` for name/phone + optional recovery email via `supabase.auth.updateUser({ email })` — amber confirmation banner on first-time add; `@akoatp.com` derived emails stripped server-side and never pre-filled), Change Password (re-auth via `signInWithPassword` then `updateUser`), Danger Zone (contact-to-delete modal); inline auto-dismiss toast; uses site design tokens), **UnifiedTimeline** (merged booking + points feed, month-grouped, exports the serializable `FeedItem` type; receives items array from `UnifiedAccount` — no internal fetching)
- `components/admin/` — **PendingRedemptionsBanner** (reads pending count from `PendingRedemptionsContext`; no own subscription), **PendingSoundAlert** (render-nothing; plays descending beep via `playNotificationBeep` when redemption pending count increases — initial load skipped), **PendingBookingsSoundAlert** (render-nothing; plays ascending beep via `playBookingBeep` when booking pending count increases — initial load skipped; same pattern as PendingSoundAlert), **RedemptionsList** (accepts `initialRequests` + `initialHistory: RedemptionRequest[]`; shows pulsing-dot "Awaiting approval" section with pending cards at top, then history table of approved/rejected/cancelled below; real-time subscription + 20s poll keep pending list live), **RedemptionRequestCard** (amber card — customer avatar initials, reward icon, ref code `#XXXXXXXX` from first 8 chars of UUID, points cost, balance; Approve green / Decline red-outlined buttons with loading spinners), AddPointsForm, **AdjustPointsForm** (manual point corrections — positive or negative, mandatory reason field), **CustomerSearch** (searches by name OR phone via `?q=`; was phone-only `type="tel"` — now `type="text"`; search submitted on form submit), **CustomersTable** (client component — accepts `Profile[]` + optional `hoursMap: Map<string, number>`; renders real `<table>` on desktop (`hidden md:block`) with columns Customer/Phone/Points/Hours/Joined/Actions, and card list on mobile (`md:hidden`); 20/page pagination with `Showing X–Y of Z` counter; customers page fetches `.limit(200)` — flag if > 200 customers and add server-side pagination), **CustomerRow** (accepts `variant: 'table' | 'card'` — `table` renders a `<tr>` for the desktop table, `card` renders the mobile div; accepts optional `hoursPlayed?: number`; **exports `getAvatarColor(name)` and `getInitials(name)` helpers** used across all admin pages for consistent colored avatar initials — color derived from name char-code hash, 8 palette options), **EarningRuleCard** (client — displays `POINTS_PER_HOUR` pts/hr earning rate with an inline edit toggle; edit saves to local state only, no backend persistence — editing the actual rate requires changing `POINTS_PER_HOUR` in `lib/points.ts`), **CustomerDetailActions** (client — wraps `TempPasswordModal` + `DeleteCustomerButton` for the customer detail right column; manages modal open state), RewardForm, **RewardAdminRow** (redesigned: Zap icon + name/description/cost/stock; ToggleRight/ToggleLeft for active toggle, Pencil for edit, Trash2 for delete; rewards page splits into Active / Inactive sections), ResetPasswordForm, DeleteCustomerButton, CreateAdminForm, StaffResetPasswordForm, DeleteStaffButton, **AdminShell** (the admin layout shell — collapsible grouped sidebar `Dashboard · Booking{Bookings, Court} · Loyalty{Points & Rewards, Requests +badge, Customers} · Content{News — admin/superadmin, Staff — superadmin-only} · Data{Backup & Export — superadmin-only}` + mobile drawer + topbar; sidebar is `fixed inset-y-0 left-0 z-30` on desktop (`position: fixed`, `width: 68 | 248px`); main content has `marginLeft` matching sidebar width **on desktop only** (applied via `md:[margin-left:var(--sidebar-w)]` + CSS custom property — mobile gets zero margin so content takes full width); both animate with `transition duration-200`; mobile drawer is already `fixed inset-y-0`; reads `usePendingRedemptions()` for the amber `--color-accent` badge on the Requests item, `"99+"` when count > 99; also reads `usePendingBookings()` for the amber badge on the Bookings item (`bookingBadge: true` on that NAV entry); superadmin-only items hidden by role; replaced the old horizontal `AdminNav`; topbar shows `LanguageToggle variant="admin"` + `LogoutButton` only — username removed, sidebar footer already shows it; sidebar footer username/role block is a `<Link href="/admin/profile">` — clicking it navigates to the admin profile page; sets `document.title` to `"(!) Mya Thida Admin"` when total pending count > 0 and `"Mya Thida Admin"` otherwise — tab-title fallback for audio-blocked notifications), **AdminProfileForm** (client — Profile (display name via `PATCH /api/profile`) + Change Password + Install App sections; neutral admin gray styling; same toast pattern as customer settings), **InstallAppCard** (see Admin Installable PWA above — native install button or platform-specific How-to modal; also registers `/sw.js`), **LogoutButton** (outline button, red border `border-red-200`, `LogOut` icon from lucide-react, `hover:bg-red-50`), **ExportPanel** (client, `/admin/export` page — superadmin-only disaster-recovery backup UI; scope tiles Full backup / This month / Pick a month (reuses `PeriodSelector`) / Custom range; downloads via `fetch` → `blob()` → temporary `<a download>`, filename read from the `Content-Disposition` response header; calls `GET /api/admin/export`)
- `components/booking/` — **NewsCarousel** (homepage news section — CSS scroll-snap horizontal carousel, 5 posts, `w-[85vw]` mobile / `w-72` desktop cards, nav dots that expand + fill `--color-primary` when active, prev/next arrows desktop-only; dots + arrows suppressed when `posts.length < 2`; `.scrollbar-hide` utility added to `globals.css`; same `NewsPostDetail` bottom-sheet/modal as `NewsCardGrid`; used in `HomeContent` — `NewsCardGrid` is still used on the `/news` page), **SiteNavbar** (top nav for `(site)` pages — mounted **once** in `app/(site)/layout.tsx`, not per-page; `active`/`back`/`mobileTitle` derived internally via `usePathname()` — no per-page props needed; desktop has Home · Book · News · **My Account** links (center nav) + `BookingLangToggle` + **User icon**: shows first name → `/account` when logged in, "Login" → `/login` when not; mobile shows icon only (BottomNav covers My Account); `initialFirstName` prop seeds auth from the layout's server render — no async `getUser()` on mount, zero auth flash; subscribes to `onAuthStateChange(SIGNED_IN/SIGNED_OUT)` for live login/logout sync; fallback async `getUser()` path runs only when `initialFirstName === undefined`), **BottomNav** (mobile-only 4-tab bar; "me" tab → `/account`), **BookingView** (client state: `cart: CartSlot[]`, `pendingSheetHour: number | null`; builds `?items=` + `?overrides=` URLs), **TimeSlotGrid** (accepts `overrideSelected?: number[]` + `onPendingClick?` props), **SlotTile** (accepts `selectedAsOverride?: boolean` — renders amber override styling; `onPendingClick` fires for pending slots), **PendingSlotSheet** (bottom sheet / modal; appears when user taps a pending slot; `hour` prop drives content; `onConfirm` adds override to cart), **ConfirmFlow** (reads `BookingGroup.overrideHours`, sends `override_request: true` for affected date groups, shows amber priority notice in step 1), **SlotLegend** (shows "tap to request" hint next to pending state), **AdminBookingsList** (server-side paginated — receives `initial`, `total`, `page`, `totalPages`, `pageSize`, `currentStatus`, `currentSub`, `currentSearch/From/To` props; URL-based navigation via `router.push()` for all filter/page changes; 4 top-level tabs: **All | Pending | Confirmed | History**; History tab shows past bookings (`booking_date < today`), cancelled, and closed; History tab has secondary pill sub-filter row: **All history | Cancelled | No-show** — active sub-tab = outlined `border-2 border-gray-900` style (distinct from primary tab dark-fill); `All` top-level tab excludes terminal statuses to avoid overlap with History; debounced search input + date-range pickers + numbered pagination controls; real-time subscription patches INSERT/UPDATE in local state — INSERT prepends to page 1 if filter matches, shows "new bookings" banner on other pages; 20s poll removed; includes "+ New Booking" button that opens `AdminNewBookingPanel`; displays `source` badge, guest name/phone fallback, expandable `internal_notes`, and `is_archived` badge per row; **slot-aware grace period**: client-side `slotGraceState(booking, todayMM, nowHourFrac)` classifies today's bookings as `active` / `grace` (slot ended but <2h ago) / `expired` (>2h past slot end) — `expired` rows are filtered out of All/Pending/Confirmed tabs entirely; `grace` rows are shown after active rows under an amber "Earlier today" sub-header at `opacity-50` with deposit toggle and cancel still active; History tab bypasses this filtering; `GRACE_HOURS=2`; **`statPending` count** in `app/(admin)/admin/bookings/page.tsx` uses the same slot-aware logic — tomorrow-and-beyond counted directly, today fetched with slots and filtered by the same `GRACE_HOURS=2` window so the stat card always matches the Pending tab; **slot time display** uses `slotRanges(hours)` — consecutive hours merge into a single `HH:MM – HH:MM` range, non-consecutive hours render as separate stacked lines; never use a raw first→last span across non-adjacent slots)
- `components/admin/booking/` — **AdminNewBookingPanel** (client, slide-in panel — desktop right drawer / mobile bottom sheet; props: `isOpen`, `onClose`, `onSuccess(ref, hadConflict)`; Section 1: phone debounce 400ms → `GET /api/customers?phone=...` → customer match or guest mode with name+phone inputs; Section 2: date picker + slot grid via `GET /api/admin/slot-availability`, 4-col × 16-row, `MAX_SLOTS=2` enforced; Section 3: deposit amount, source pills `phone/walk_in/other`, deposit-received toggle, internal notes; full state reset on `isOpen` toggle; `todayYangon()` uses `Intl.DateTimeFormat Asia/Yangon` for default date; submits to `POST /api/admin/bookings`; past slots — `date < todayISO` or `hour < currentHour` on today (Myanmar TZ, strict `<`) — render `opacity-50` + "Past" label and an amber banner for past dates, but remain **fully clickable** for walk-in/phone recording; do NOT disable past slots or add server-side time filtering to the admin slot endpoint)
- `components/admin/analytics/` — superadmin-only chart components (all `'use client'`): `DashboardPeriodSection` (client wrapper that owns `useTransition` for period navigation; renders: period selector row → Bookings `SectionDivider` → `BookingKpiCards` → `BookingChartsRow` → Loyalty `SectionDivider` → `EarningRuleCard` → 4 amber `LoyaltyKpiCard`s → `ChartsSection`; all booking/loyalty components receive `loading={isPending}` for skeleton during period transitions; prevents full-page `loading.tsx` on month/year change), `ChartsSection` (dynamic-imports charts with `ssr:false`; accepts `isPending` prop to replace chart areas with `ChartSkeleton`; points bar chart is period-scoped; status donut is **all-time**), `PointsBarChart` (points issued vs redeemed, daily across the selected month), `StatusDonut` (redemption status breakdown — **all-time**, not period-filtered), `TopRewardsBar` (top 5 rewards by approvals in the selected month), `TopCustomersBar` (unused by dashboard — top customers now rendered as `TopCustomersWithProgress` in the all-time Overview section), `PeriodSelector` (month + year dropdowns; accepts `onNavigate` callback prop so parent can wrap navigation in `startTransition`; accepts `disabled` prop to lock dropdowns during transition), `PeriodLabel` (renders the selected period as an uppercase "MONTH YEAR" heading). All use Recharts + `useLanguage()` for i18n.
...
**Dashboard layout (superadmin):** h1 title → `DashboardPeriodSection` (period selector + Bookings section + Loyalty section, all period-filtered) → `SectionDivider` "Overview" → `grid-cols-2` row: **Points Issued all-time** (amber left-accent, `pointsIssuedAllTime`) + **Total Customers all-time** (blue `#2563eb` left-accent, `allTimeCustomerCount`, subtitle shows `newCustomersPeriodCount` joined in selected period) → `grid-cols-1 sm:grid-cols-2` row: **Top Customers card** (all-time by `total_points`, `TopCustomersWithProgress`) with a **Most Redeemed Rewards sub-section** inside (divider + top 3 rewards by all-time approved redemption count, proportional amber mini-bars, computed client-side from up to 500 approved requests; empty state if none) | **Recent Activity Feed**. **Dashboard layout (admin, non-superadmin):** h1 → Bookings `SectionDivider` + `BookingKpiCards` + `BookingChartsRow` (current week/month, no period filter) → Loyalty `SectionDivider` + `EarningRuleCard` + 4 inline amber KPI cards (Points Outstanding / Pending Requests / Approvals This Month / Total Redeemed) → `grid-cols-1 sm:grid-cols-2` bottom row: Total Members card + Recent Activity Feed.
...
- **All-time (NOT affected by the filter):** Points Issued + Total Customers KPI cards; Top Customers by `total_points` (top 5) with Most Redeemed Rewards sub-section (top 3 by all-time approved redemption count, grouped client-side); Recent Activity Feed; Status Donut. (The pending count for `PendingRedemptionsBanner` is fetched once in `app/(admin)/layout.tsx` as `initialCount` for `PendingRedemptionsContext` — not a dashboard page query.)
- **Period-scoped (`[periodStart, periodEnd)` half-open range):** Booking count / Revenue / New Customers KPI cards, court utilization chart (weekly W1–W5), peak hours chart, Loyalty KPI cards (Points Issued / Pending Redemptions / Approvals), daily points bar chart, Top Rewards (approvals). Period-scoped queries use different timestamp columns per table: `profiles` and `point_transactions` filter on `created_at`; `redemption_requests` filters on **`requested_at`** (that table has no `created_at` column — it uses `requested_at` as the submission timestamp). Mixing up column names causes queries to silently return null (Supabase PostgREST drops rows when the filter column doesn't exist), making charts show "no data."
...
| `POST /api/admin/bookings/restore` | superadmin | Bulk restore archived bookings — body: `{ ids: string[] }`; sets `is_archived=false` |
| `POST /api/admin/bookings/hard-delete` | superadmin | Permanently delete already-archived bookings — body: `{ ids: string[] }`; guard: only deletes rows where `is_archived=true` |
| `GET /api/admin/bookings/purge` | superadmin | Preview purge count — `?olderThanDays=30\|90\|180\|365`; returns `{ count }` of old cancelled/confirmed rows |
| `POST /api/admin/bookings/purge` | superadmin | Bulk hard-delete old bookings — body: `{ olderThanDays: 30\|90\|180\|365 }`; deletes cancelled/confirmed rows older than cutoff |
| `GET /api/admin/export` | superadmin | Disaster-recovery backup — query: `scope=all\|month\|range` (+`month`/`year` or `from`/`to`). Returns a multi-sheet `.xlsx` (README, Customers, Rewards, Bookings, Booking Slots, Point Transactions, Redemptions, Court Closures) built with `exceljs` via `createServiceClient()`. Customers + Rewards are always exported in full regardless of scope (current-state snapshot); the other five sheets are date-filtered per-table (`booking_date` / `created_at` / `requested_at` / `closure_date`). Batches reads with `.range()` in pages of 1000 to work around the PostgREST row cap. |
...
| `POST /api/cms` | admin/superadmin | Create CMS post (news/promotion/league/event) — body: `slug`, `category`, `title`, `title_my`, `excerpt`, `excerpt_my`, `manual_image_url` (optional, Cloudinary URL set by upload zone), `published` |
| `PUT /api/cms/[id]` | admin/superadmin | Update CMS post fields (preserves `published_at` on publish) |
| `DELETE /api/cms/[id]` | admin/superadmin | Delete CMS post |
| `POST /api/cms/upload-image` | admin/superadmin | Upload cover image to Cloudinary — multipart/form-data with `file` field (JPEG/PNG/WebP, max 5 MB); returns `{ url: string }` (Cloudinary `secure_url`); folder `myathida-futsal/cms`, width 800 limit, quality/format auto. `CLOUDINARY_API_SECRET` never leaves the server. |
...
- `rr_customer_cancel` UPDATE policy tightened + `trg_cancel_columns_immutable` trigger added — customers can only flip `status` to `cancelled` on their own pending requests; all other columns (`customer_id`, `reward_id`, `requested_at`, `resolved_at`, `resolved_by`, `notes`, `points_cost_snapshot`) are reset to their pre-update values for non-admin actors (`rls-redemption-cancel-fix.sql` + `redemption-cancel-snapshot-fix.sql`)
