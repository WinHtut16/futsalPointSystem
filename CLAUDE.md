...
```
Tables enabled: `bookings` (`booking-system-migration.sql`), `profiles` + `redemption_requests` (`realtime-publication-migration.sql`). REPLICA IDENTITY FULL set for all three in `realtime-publication-migration.sql`.
...
| `POST /api/admin/bookings/archive` | admin/superadmin | Bulk soft-archive bookings — body: `{ ids: string[] }` (max 100 UUIDs); sets `is_archived=true`; two-step safety before hard-delete |
