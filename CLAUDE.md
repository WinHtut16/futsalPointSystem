...
| `GET /api/account/history` | authenticated customer | Paginated history feed — params: `filter` (`all`\|`bookings`\|`points`), `page` (0-indexed). For `all`: fetches `offset+limit+1` from both tables, merges, sorts, slices. Returns `{ items: FeedItem[], hasMore: boolean }`. |
