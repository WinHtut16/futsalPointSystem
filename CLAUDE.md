...
| `useBookingSlotRealtime` (hook) | broadcast channel `booking-slot-updates` | `slot_changed` broadcast event | Customer booking page — triggers `refreshDate(dateISO)` which silently re-fetches `/api/bookings/availability?date=…` (no loading state) and patches `dayInfoState`/`calDataState` in `BookingView`; event-driven only, no polling; uses Realtime broadcast (not `postgres_changes`) so events are delivered to anonymous users regardless of RLS |
