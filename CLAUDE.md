...
- **DB:** `booking-system-migration.sql` adds `bookings`, `booking_slots` (with `active` mirror column + `uq_active_slot_per_hour` partial unique index as the race guard; kept in sync with parent booking status via the `sync_booking_slots_active` trigger), `court_closures`, `cms_posts` (promotions = `category='promotion'`; `body_md`/`cover_url` columns exist but are no longer written — use `source_url` + `manual_image_url` instead, added by `cms-simplify-migration.sql`). Adds `'booking'` to the `point_transactions.transaction_type` check. `create_booking_transaction()` RPC inserts a booking + its slots atomically and generates the `MYF-YYYY-NNNN` ref via `booking_ref_seq`. `bookings` is in the realtime publication. `pending-override-migration.sql` adds `override_request BOOLEAN NOT NULL DEFAULT FALSE` to `bookings` and creates `create_override_booking_transaction()` RPC — identical to `create_booking_transaction` but inserts slots with `active=false`, bypassing the `uq_active_slot_per_hour` partial unique index so an override booking can hold the same slot as an existing pending booking. When admin confirms an override booking, `PATCH /api/bookings/[id]` cancels conflicting pending bookings first (trigger sets their slots `active=false`), then confirms the override (trigger sets its slots `active=true`).
...
| `POST /api/cms` | superadmin | Create CMS post (news/promotion/league/event) — body: `slug`, `category`, `title`, `title_my`, `excerpt`, `excerpt_my`, `source_url` (required, external link), `manual_image_url` (optional), `published` |
