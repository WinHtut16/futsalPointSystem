...
export default function CmsPostList({ initial }: { initial: CmsPostRow[] }) {
  ...
  if (rows.length === 0) {
    return <p className="rounded-2xl bg-white p-8 text-center text-sm text-gray-400 shadow-sm">No posts yet.</p>
  }
      ...
      {rows.map((p) => (
        <div key={p.id} className="flex items-center justify-between rounded-2xl bg-white p-4 shadow-sm">
          ...
          <div className="flex shrink-0 gap-2">
            <Link href={`/admin/cms/${p.id}/edit`} className="rounded-lg border border-gray-300 p-2 text-gray-600 hover:bg-gray-50" aria-label="Edit">
              <Pencil className="h-4 w-4" />
            </Link>
            <button onClick={() => remove(p.id)} disabled={busy === p.id} className="rounded-lg border border-gray-300 p-2 text-red-600 hover:bg-red-50" aria-label="Delete">
